 ...
 * 无条件信任的代价：任何人只要带上 `X-Forwarded-For: <随机IP>` 就能
 *   1) 让「同一 IP 连续失败 N 次就拒绝」的防爆破完全失效（每次换个 IP），
 *   2) 用海量伪造 IP 把失败记录表撑到内存耗尽。
 * 因此默认只看 socket 地址；确实部署在 nginx / CDN 后面时再开 trustProxy。
 */
