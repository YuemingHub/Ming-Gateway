...
class GatewayServer {
  ...
  handleAdmin(req, res, pathname, url) {
    ...
    if (pathname === '/__gw/api/channel/reorder') {
      if (req.method !== 'POST') return sendJson(res, 405, { ok: false, error: '需要 POST' });
      return this.readJsonBody(req, res, 1 << 20).then((read) => {
        if (!read.ok) return;
        return this.handleChannelReorder(req, res, read.body);
      });
    }
   ...
   * 调整某个组内的消耗顺序。
   *
   * 入参 { group, ids }：ids 必须是该组**全部渠道**的新顺序（页面把当前整组按显示顺序提交）。
   * 之所以要求整组提交、而不是「把某条上移一位」：一次请求就把顺序钉死，
   * 重复点击 / 并发不会产生歧义，服务端也不必去猜「上移一位」的语义。
   * 落盘走 persistChannels（校验 → 写文件 → 热更新），顺序立刻对数据面生效。
   */
  handleChannelReorder(req, res, body) {
    const group = String((body && body.group) || '').trim().toUpperCase();
    const ids = body && Array.isArray(body.ids) ? body.ids.map((x) => String(x)) : null;

    if (!group || !this.config.groups[group]) {
      return sendJson(res, 400, { ok: false, error: '分组不存在：' + group });
    }
    if (!ids) return sendJson(res, 400, { ok: false, error: '缺少 ids 数组' });

    const stored = this.storedChannels();
    const inGroup = stored.filter((c) => String(c.group || '').trim().toUpperCase() === group);
    if (!inGroup.length) return sendJson(res, 404, { ok: false, error: group + ' 组下没有渠道' });

    const cur = new Set(inGroup.map((c) => c.id));
    const want = new Set(ids);
    if (ids.length !== inGroup.length || want.size !== ids.length) {
      return sendJson(res, 400, {
        ok: false,
        error: `ids 必须不重不漏地覆盖 ${group} 组全部 ${inGroup.length} 个渠道（收到 ${ids.length} 个）`,
      });
    }
    for (const id of ids) {
      if (!cur.has(id)) return sendJson(res, 400, { ok: false, error: `渠道 ${id} 不属于 ${group} 组` });
    }

    const orderOf = new Map(ids.map((id, i) => [id, i]));
    const list = stored.map((c) => (orderOf.has(c.id) ? Object.assign({}, c, { order: orderOf.get(c.id) }) : c));
    const r = this.persistChannels(list);
    if (!r.ok) return sendJson(res, 400, { ok: false, error: r.errors.join('; '), errors: r.errors });

    this.log('info', `渠道顺序已调整：${group} 组 → ${ids.join(' > ')}`);
    return sendJson(res, 200, { ok: true, group, ids });
  }
  ...
  handleChannelSave(req, res, body) {
    ...
    // 换了组：原组内的序号在新组里没有意义，清掉让它排到新组末尾（重编号会补上）
    if (!created) {
      const oldGroup = String(list[idx].group || '').trim().toUpperCase();
      const newGroup = String(next.group || '').trim().toUpperCase();
      if (oldGroup !== newGroup) delete next.order;
    }
  ...
  async getUpstreamModels(ch, timeoutMs) {
    ...
    if (r.status < 200 || r.status >= 300) {
      ...
      // 鉴权失败时把上游原话一并带上：它比「API Key 可能无效」精确得多
      const reason = authFail ? upstreamReason(r.text) : '';
      const err = new Error(
        authFail
  ...
  statusPayload(req) {
    ...
    const channels = this.config.channels.map((c) => {
      ...
      return {
        ...
        weight: c.weight,
        priority: c.priority,
        // 组内消耗顺序：状态页的「顺序」列与列表排序都读这个字段。
        // 少写它会让每行都显示成第 1 位、排序退化成按 id —— 页面直接失效。
        order: c.order,
 ...
 * 从上游的错误响应里挖出「人话」，给排查用。
 *
 * 各家报错形状不一：方舟 / OpenAI 风格是 { error: { code, message } }，
 * 也有把 message 直接放顶层的。挖不出来就退回截断后的原文。
 *
 * 为什么要这么干：以前 401 一律翻译成「API Key 可能无效、过期或权限不足」，
 * 把上游的原话吞掉了 —— 而原话往往精确得多（例如方舟会说
 * "The API key format is incorrect"，一眼就能看出是密钥类型不对，不是过期）。
 */
function upstreamReason(text) {
  ...
  try {
    ...
    const e = j && j.error;
    const msg =
      (e && typeof e === 'object' && (e.message || e.msg)) ||
      (typeof e === 'string' ? e : '') ||
      (j && (j.message || j.msg || j.detail)) ||
      '';
    if (msg) return oneLine(msg, 200);
  } catch (_) {
    // 不是 JSON，直接用原文
  }
  return raw;
